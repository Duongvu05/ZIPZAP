...
```sh
cd src;
python gen_seq.py --bizdate=zipzap_exp
```
...
```sh
python run_pretrain.py --bizdate="zipzap_exp" \
                       --ckpt_dir="zipzap_exp"  
```
...
```sh
python run_embed.py --bizdate="zipzap_exp" \
                       --init_checkpoint="zipzap_exp/epoch_300.pth"
```
