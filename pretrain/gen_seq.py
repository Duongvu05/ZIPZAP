...
parser.add_argument("--data_dir", type=str, default="/home/phinn/BERT4ETH/ZIPZAP/data/", help="data directory.")
...
def main():
    ...
    print("Add phisher account transactions.")
    phish_in = open(os.path.join(args.data_dir, "phisher_transaction_in.csv"), "r")
    phish_out = open(os.path.join(args.data_dir, "phisher_transaction_out.csv"), "r")
    eoa2seq_in, eoa2seq_out = load_data(phish_in, phish_out)
    if args.dup:
        eoa2seq_agg_phisher = seq_duplicate(eoa2seq_in, eoa2seq_out)
    else:
        eoa2seq_agg_phisher = seq_generation(eoa2seq_in, eoa2seq_out)
    eoa2seq_agg.update(eoa2seq_agg_phisher)
