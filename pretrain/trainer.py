...
def gather_indexes(sequence_tensor, positions , device):
    """
    Gathers the vectors at the specific positions over a minibatch.
    """
    batch_size, seq_length, width = sequence_tensor.size()
    flat_offsets = torch.arange(0, batch_size, dtype=torch.long , device=device) * seq_length
    flat_offsets = flat_offsets.unsqueeze(-1)  # reshape to [batch_size, 1]
    flat_positions = (positions + flat_offsets).view(-1).to(device)
...
class BERT4ETHTrainer(nn.Module):
    def __init__(self, args, vocab, model, data_loader):
        ...
        self.fc = nn.Linear(args.hidden_size*2, args.hidden_size).to(self.device)
    ...
    def calculate_loss(self, batch):
        ...
        input_mask = batch[6]
        labels = batch[7]
        covered_indexes = batch[8]

        h = self.model(input_ids, counts, values, io_flags, positions).to(self.device) # B x T x V
        

        input_tensor = gather_indexes(h,covered_indexes ,self.device)
        # Transformation
        ...
        pos_output_weights = self.model.embedding.token_embed(labels).squeeze() # positive embedding
        neg_output_weights = self.model.embedding.token_embed(neg_ids).squeeze() # negative embedding
    ...
    def infer_embedding(self, ):
        ...
        with torch.no_grad():
            for batch_idx, batch in enumerate(tqdm_dataloader):
                ...
                max_embedding = torch.max(h, dim = 1)[0]
                # mean embedding  
